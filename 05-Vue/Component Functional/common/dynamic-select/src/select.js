...
export default {
  ...
  computed: {
    ...
    requestOption() {
      const paramsKey = this.method.toUpperCase() === 'GET' ? 'params' : 'data'
      return {
        url: this.url,
        method: this.method,
  ...
  methods: {
    ...
    handleChangeEvent(val) {
      ...
      if (this.onEvents.dataChange) {
        ...
        if (this.bindAttrs.multiple) {
          data = []
          if (val && val.length) {
            data = this.optionsData.filter(i =>
              val.includes(i[this.optionsProps.value])
            )
          }
        } else {
    ...
    async getOptionsData(query) {
      return new Promise((resolve, reject) => {
        ...
        this.loading = true
        request({ ...requestOption, ...queryParam })
          .then(res => {
            this.loading = false
